...
try {
  const res = await axios.get(`https://hacker-news.firebaseio.com/v0/${choice}.json`);
  const data = res.data;
  ...
  const storyPromises = slicedData
    ...
    .map((id_1: number) => {
      ...
      return axios.get(`https://hacker-news.firebaseio.com/v0/item/${id_1}.json`).then(res_1 => res_1.data);
    });
